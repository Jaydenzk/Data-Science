...
with open('xgb_reg_pickle1', 'wb') as f:
     pickle.dump(xgb_reg, f)

## plot feature importances
ft_weights_xgb_reg = pd.DataFrame(xgb_reg.feature_importances_, columns=['weight'], index=X_train.columns)
ft_weights_xgb_reg.sort_values('weight', inplace=True)
ft_weights_xgb_reg[::-15]
...
two_features = X[['bathrooms', 'bedrooms']]
...
xgb_reg_two_features = xgb.XGBRegressor()
xgb_reg_two_features.fit(X_train, y_train)
training_preds_xgb_reg = xgb_reg_two_features.predict(X_train)
val_preds_xgb_reg = xgb_reg_two_features.predict(X_test)
...
with open('xgb_reg_two_features', 'wb') as pickle_file:
  pickle.dump(xgb_reg_two_features, pickle_file)

y

X = transformed_df[['bedrooms', 'bathrooms', 'accommodates', 'instant_bookable', 'minimum_nights', 'maximum_nights', 'high_end_electronics', 'high_end_appliances', 'kitchen_luxury', 'child_friendly', 'privacy', 'free_parking', 'smoking_allowed','pets_allowed']]
...
xgb_reg_fourteen_features1 = xgb.XGBRegressor()
xgb_reg_fourteen_features1.fit(X_train, y_train)
training_preds_xgb_reg = xgb_reg_fourteen_features1.predict(X_train)
val_preds_xgb_reg = xgb_reg_fourteen_features1.predict(X_test)
...
'''with open('xgb_reg_fourteen_features1', 'wb') as pickle_file:
  pickle.dump(xgb_reg_fourteen_features1, pickle_file)
  '''
